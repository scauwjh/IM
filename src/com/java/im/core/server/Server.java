...
public class Server {
	...
	static {
		...
		try {
			InputStream in = Thread.currentThread().getContextClassLoader()
					.getResourceAsStream("im.properties");
			...
			logger.info("Read properties from im.properties");
		} catch (IOException e) {
	...
	public void server() throws IOException {
		...
		KeepAliveFilter hbFilter = new KeepAliveFilter(skmf,
				IdleStatus.BOTH_IDLE, hbHandler);
		...
		acceptor.bind(new InetSocketAddress(Constant.TEXT_PORT)); // throw an
																	// IOException
		acceptor.bind(new InetSocketAddress(Constant.IMAGE_PORT)); // throw an
																	// IOException
		System.out.println("text port " + Constant.TEXT_PORT
				+ " is listening...");
		System.out.println("image port " + Constant.IMAGE_PORT
				+ " is listening...");
	}
